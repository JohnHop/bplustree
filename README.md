...
L'obiettivo del progetto è quello di fornire una implementazione in C++ degli alberi B+Tree più semplice possibile e facilmente comprensibile, al costo di scrivere codice qualitativamente molto lontano dall'utilizzo in un ambiente professionale.
...
Per gli alberi B-Plus Tree, tutte le chiavi (e i corrispettivi valori) devono trovarsi necessariamente nei nodi foglia mentre i nodi interni, che contengono solamente le chiavi, servono solo per "instradare" la funzione di ricerca verso il figlio che contiene la chiave da ricercare.
...
Nel file `bplustree.cpp` la funzione ha la firma `std::pair<Node*,int> BTree::search(const char key)` ed è sufficientemente commentata.
...
Le differenza rispetto alla versione già vista per gli alberi B-Tree sono evidenziate dal commento `//* B+Tree ...`. Cominciamo con

```C++
y->key_count = y->leaf ? t : t-1;
```

che dovrebbe eliminare o meno la mediana dal nodo _`y`_ (osserva, l'eliminazione avviene decrementando il contatore delle chiavi). Questo a causa della netta differenza tra nodi interni (_router_) e foglia degli alberi B+Tree:

| B-Tree                                                  | B-Plus Tree                                                                                                    |
|---------------------------------------------------------|----------------------------------------------------------------------------------------------------------------|
| La mediana "sale" nel _`parent`_ e viene eliminata da _`y`_ | La mediana "sale" nel _`parent`_ ma viene eliminata da _`y`_ <u>soltanto se</u> quest'ultimo è un nodo interno (_router_) |
...
L'algoritmo di eliminazione è significativamente più complicato. Ma andiamo per ordine.

L'algoritmo utilizzato da Cormen per i B-Tree è del tipo _single pass down_. Cito:

> The B-TREE-DELETE procedure then acts in one downward pass through the tree,
without having to back up.

Invece, per i B-Plus Tree ho utilizzato il meccanismo opposto suggerito da Kerttu nei suoi appunti, ovvero
...
2. quindi, si "risale" l'albero ribilanciando, se necessario, man mano che si _risale_ lo stack delle chiamate ricorsive effettuate precedentemente
...
2. non bisogna aggiornare nessun nodo interno (_router_) in seguito all'eliminazione
...
Supponiamo di avere un albero di ordine (nella definizione di Cormen) _`t`_. Se a causa della eliminazione

1. il nodo corrente ha un numero di chiavi inferiori a _`t-1`_, e
2. un nodo vicino possiede almeno _`t`_ chiavi

allora è possibile prendere dal vicino una chiave in modo da conservare le proprietà dell'albero. L'operazione è detta _rotation_ e differisce leggermente nel caso in cui la chiave viene presa dal vicino a destra oppure da quello a sinistra, con conseguenze anche sull'aggiornamento di una possibile chiave in un nodo interno (che rende il tutto più complicato).

La funzione ha la firma `void BPlusTree::rotate(Node* parent, const int y_pos_in_parent)` ed è ben commentata. L'operazione avviene etichettando con _`y`_ il nodo che deve ricevere una chiave da un vicino etichettato a sua volta con _`s`_. Per rendere il codice più chiaro, sono state utilizzate alcune funzione "di supporto" quali
...
Il punto più critico della funzione è probabilmente l'ultimo, ovvero il 4°, che fa utilizzo della funzione `update_key()` necessaria per aggiornare i valori dei _link_ in seguito alle modifiche causate dalla rotatazione.
...
Nel file `main.cpp` è presente una breve sequenza per testare il funzionamento dell'albero.
...
L'obiettivo del repo non è quello di presentare codice di qualità a livello di produzione ma solo un esempio facilmente comprensibile (mi auguro) di implementazione di un B+Tree.